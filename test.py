...
model = OpticsCorrection(patch_size=patch_size, overlap_percentage=overlap_percentage,
                         sigma_b=sigma_b, c=c, polyblur_iteration=polyblur_iteration,
                         ker_size=ker_size)
...
imsave(os.path.join(savefolder, '%s_original.png' % name), img)
imsave(os.path.join(savefolder, '%s_corrected.png' % name), img_corrected)
